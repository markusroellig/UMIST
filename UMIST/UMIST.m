...
in13C18O::usage="in13C18O[mol] takes a chemical species mol and returns a list of all possible 13C and 18O isotopologues."
...
carbonize2[x_String] :=
    StringReplace[StringReplaceList[insertOne2[x],ruleC2],"1"->""]
SetAttributes[carbonize2,Listable]
deuteratePlus[y_] :=
    StringReplace[deuterate[y],x_~~EndOfString:>x<>charge[y]]
oxygenizePlusSingle[y_] :=
    StringReplace[oxygenizeSingle[y],x_~~EndOfString:>x<>charge[y]]
carbonizePlusSingle[y_] :=
    StringReplace[carbonizeSingle[y],x_~~EndOfString:>x<>charge[y]]

carbonizePlusChain[y_] :=
    StringReplace[carbonizeChain[y],x_~~EndOfString:>x<>charge[y]]
...
cleanUpMN[list_List] := Module[{rest, refline, debug},
 ...
 Block[{updatedData, newData2, showR, len, data, monitorCell},
  updatedData = 
   Map[Flatten@{#[[{1, 2}]], 
     ...
     Table[
      If[showR@i != {},(*to catch missing reactions*)
       Which[
        ...
        Sequence @@ (showR@i)], 
       Print["Missing reaction " <> ToString@i];
       log = {"Missing reaction " <> ToString@i, log};], {i, 1, 
       Length[list]}]
